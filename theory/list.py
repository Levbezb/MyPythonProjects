...
first = lst[0][0]   # Чтобы обратиться к элементу списка в списке, нужно использовать такую индексацию

ln = len(lst)       # Результатом функции len() будет количство элементов основного списка, в данном случае 3
                    # Чтобы посчитать элементы всех списков, нужно использовать цикл

'''Функции min() и max() могут работать и со списками. Если этим функциям передается несколько списков, то целиком возвращается один
 из переданных списков. При этом сравнение происходит поэлементно: сначала сравниваются первые элементы списков. Если они не равны, 
 то функция min() вернет тот список, первый элемент которого меньше, max() – наоборот. Если первые элементы равны, то будут сравниваться 
 вторые и т. д.'''

mn = min(lst)       # [1, 2, 3]
mx = max(lst)       # [7, 8, 9]

# Методы списков работают одинаково как с обычными так и с вложенными
